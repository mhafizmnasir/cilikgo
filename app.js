...
$('#loginBtn').onclick=async()=>{
  if(!fb) return toast('Mode demo aktif. Sambungkan Firebase untuk login sebenar.');
  try{ await fb.signInWithEmailAndPassword(fb.auth,$('#loginEmail').value,$('#loginPassword').value); $('#loginModal').close(); toast('Berjaya log masuk.'); }catch(e){ toast('Log masuk gagal: '+e.message); }
};

$('#forgotBtn').onclick=async()=>{
  const email=$('#loginEmail').value.trim(); if(!email) return toast('Masukkan e-mel terlebih dahulu.');
  if(!fb) return toast('Demo: fungsi reset akan menggunakan Firebase Auth.');
  try{ await fb.sendPasswordResetEmail(fb.auth,email); toast('E-mel reset kata laluan telah dihantar.'); }catch(e){ toast(e.message); }
};
