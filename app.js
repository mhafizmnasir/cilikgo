...
function celebrate(){
  ...
  burst.className='quiz-celebrate-burst mega';
  const pieces=['🎉','✨','⭐','🌟','🎊','💫','⭐','✨','🎉'];
  const arcs=[[-122,-72],[-88,-108],[-54,-92],[-20,-118],[0,-132],[24,-116],[58,-94],[92,-108],[126,-74]];
  burst.innerHTML=`<div class="quiz-celebrate-center">🎉 Hebat! 🎉</div>${pieces.map((icon,i)=>`<i class="quiz-celebrate-piece" style="--tx:${arcs[i][0]}px;--ty:${arcs[i][1]}px;animation-delay:${(i*0.04).toFixed(2)}s">${icon}</i>`).join('')}`;
  shell.appendChild(burst);
  setTimeout(()=>burst.remove(),1350);
}
