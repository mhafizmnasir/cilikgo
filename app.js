...
async function callAdminSubscriptionAction(targetUid,action){
  ...
  const res=await fetch(`${FUNCTIONS_BASE_URL}/adminManageSubscription`,{
    ...
    body:JSON.stringify({targetUid,action,requestId})
  });
  const data=await res.json().catch(()=>({}));
  if(!res.ok)throw new Error(data.error||'Gagal mengemas kini langganan.');
  return data;
}
...
async function renderAdminSubscriptions(){
  ...
  try{
    const [usersSnap,ordersSnap,settings]=await Promise.all([
    ...
    const bind=()=>{
      document.querySelectorAll('.sub-manage').forEach(btn=>btn.onclick=async()=>{
        ...
        try{
          const result=await callAdminSubscriptionAction(u.id,action);
...
async function renderAdmin(p){
  ...
  const [users,children,allProgress,orders,commissions,manualPaymentSettings]=await Promise.all([
    safeDocs('users'),safeDocs('children'),safeDocs('progress'),safeDocs('orders'),safeDocs('commissions'),loadManualPaymentSettings(true)
  ]);
  const progress=allProgress.filter(r=>Number(r.year)>=1&&r.subject);
  ...
  const views={
    ...
    commissions:()=>`${head('Komisen','Rekod affiliate manual')}<div class="stat-grid"><div class="stat"><small>Rekod</small><b>${commissions.length}</b></div><div class="stat"><small>Jumlah</small><b>RM${commissionTotal.toFixed(2)}</b></div><div class="stat"><small>Pending</small><b>${commissions.filter(c=>c.status==='pending').length}</b></div></div>${commissions.length?`<div class="table-wrap"><table class="table"><tr><th>Agent</th><th>Jualan</th><th>Jenis</th><th>Komisen</th><th>Status</th></tr>${commissions.map(c=>{const a=users.find(u=>u.id===c.agentUid);return `<tr><td>${esc(a?.name||c.agentUid||'-')}</td><td>RM${Number(c.saleAmount||0).toFixed(2)}</td><td>${esc(c.rateLabel||(c.ratePercent?c.ratePercent+'%':'Tetap'))}</td><td>RM${Number(c.amount||0).toFixed(2)}</td><td>${statusBadge(c.status)}</td></tr>`}).join('')}</table></div>`:empty('Belum ada komisen.')}`,
    ...
    settings:()=>`${head('Tetapan','Pembayaran manual & promosi')}<div class="settings-grid"><div class="setting-card"><b>Promosi Lifetime</b><strong>${manualPaymentSettings.lifetimePromoActive!==false?'AKTIF':'TAMAT'}</strong><small>RM45 sekali bayar untuk pengguna baharu</small></div><div class="setting-card"><b>Selepas promosi</b><strong>RM45</strong><small>3 bulan · kemudian RM15/bulan</small></div><div class="setting-card"><b>Komisen Agent</b><strong>RM30 / RM10</strong><small>Pelanggan baharu / renewal bulanan</small></div></div><section class="admin-manual-payment-settings"><div class="dash-head compact"><div><small>PEMBAYARAN MANUAL</small><h3>Akaun bank & WhatsApp Admin</h3></div></div><div class="manual-settings-grid"><label><span>Nama Bank</span><input id="manualBankName" value="${esc(manualPaymentSettings.bankName||'')}" placeholder="Contoh: Maybank"></label><label><span>Nama Pemilik Akaun</span><input id="manualAccountName" value="${esc(manualPaymentSettings.accountName||'')}" placeholder="Nama seperti pada akaun bank"></label><label><span>Nombor Akaun</span><input id="manualAccountNumber" value="${esc(manualPaymentSettings.accountNumber||'')}" placeholder="Nombor akaun bank"></label><label><span>WhatsApp Admin</span><input id="manualAdminWhatsapp" value="${esc(manualPaymentSettings.adminWhatsapp||'')}" placeholder="60123456789"></label><label class="manual-promo-switch"><input id="manualLifetimePromo" type="checkbox" ${manualPaymentSettings.lifetimePromoActive!==false?'checked':''}><span><b>Promosi Lifetime RM45 aktif</b><small>Tutup suis ini apabila promosi tamat. Pelanggan baharu selepas itu menerima 3 bulan pada RM45.</small></span></label></div><button class="btn primary" id="saveManualPaymentSettings">Simpan Tetapan Pembayaran</button></section><div class="dash-note">📧 Selepas Admin mengaktifkan langganan, Firebase Function akan memasukkan e-mel pengaktifan ke queue. Tetapkan SMTP Gmail App Password sebelum deploy Functions.</div>`
  };
