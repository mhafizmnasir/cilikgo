...
async function renderAgent(p){
  ...
  const views={
    ...
    settings:()=>`<div class="dash-head portal-page-head"><div><small>Akaun Agent</small><h2>Tetapan</h2><p>Urus referral dan akaun bank untuk pembayaran komisen mingguan.</p></div><span class="badge">Agent</span></div>
      ...
      <section class="agent-bank-settings">
      ...
      <div class="dash-note">🔒 Kata laluan dan keselamatan akaun diurus melalui Firebase Authentication.</div>`
  };
...
async function renderAdmin(p,initialView='overview'){
  ...
  const views={
    overview:()=>`${head('Overview')}<div class="admin-stat-grid">
      ...
      <div class="stat"><small>Jualan dibayar</small><b>${formatMoney(sales)}</b></div><div class="stat"><small>Keuntungan Admin</small><b>${formatMoney(adminProfit)}</b></div>
      </div><div class="admin-finance-strip"><div><small>JUALAN KASAR</small><b>${formatMoney(sales)}</b></div><div><small>KOMISEN AGENT</small><b>${formatMoney(commissionTotal)}</b></div><div><small>PERLU BAYAR SABTU</small><b>${formatMoney(commissionPendingTotal)}</b><span>${esc(saturdayLabel)}</span></div><div class="profit"><small>UNTUNG ADMIN</small><b>${formatMoney(adminProfit)}</b></div></div><div class="admin-two-col"><div><h3>Akaun terkini</h3>${users.length?`<div class="table-wrap"><table class="table"><tr><th>Nama</th><th>Role</th><th>Status</th></tr>${users.slice(-8).reverse().map(u=>`<tr><td>${esc(u.name||u.email||'-')}</td><td>${statusBadge(u.role)}</td><td>${statusBadge(u.subscriptionStatus||'n/a')}</td></tr>`).join('')}</table></div>`:empty('Tiada akaun.')}</div>
